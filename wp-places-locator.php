...
function wppl_save_data($post_id) {
	...
	if ($old_address_arg != $address_arg) {
		...
		$wpdb->replace( $wpdb->prefix . 'places_locator', array( 'post_id' => $post->ID, 'post_type' => $_POST['post_type'] ,'lat' => $lat , 'long' => $long, 'post_title' => $_POST['post_title'], 'post_status' => $_POST['post_status'], "address" => $address_arg, 'phone' => $_POST[$wppl_meta_box['fields'][6]['id']], 'fax' => $_POST[$wppl_meta_box['fields'][7]['id']], 'email' => $_POST[$wppl_meta_box['fields'][8]['id']], 'website' => $_POST[$wppl_meta_box['fields'][9]['id']]));
		} else {
