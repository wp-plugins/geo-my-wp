jQuery(document).ready(function() {
    ...
    /* locate user */
    jQuery('#locate-me-bp').click(function(){
    	jQuery("#wppl-ajax-loader-locate").show();
  		getLocationBP();
  	}); 
	...
	function breakAddress(address) {
		jQuery("#address-tab-wrapper :text").val('');
		for ( x in address ) {
         	...
         	if (address[x].types == 'route') {
          	...
          	if (address[x].types == 'administrative_area_level_1,political') {
          		if(address[x].short_name != undefined) {
          			...
          			document.getElementById("wppl_state_short").value = stateShort;
          		}		
          	}
          	...
          	if (address[x].types == 'country,political') {
          		if(address[x].long_name != undefined) {
          			var country = address[x].long_name;
          	...
          	jQuery("#wppl-ajax-loader-locate").hide();		
        }
	}
	...
	function returnAddress(gotLat, gotLong) {
		...
		geocoder.geocode({'latLng': latlng, 'region':   'es'}, function(results, status) {
      		if (status == google.maps.GeocoderStatus.OK) {
        		...
        		jQuery("#wppl-location-form :text").val('');
      		}
   		});
	} 
    ...
    function saveLocation() {
		...
		jQuery.ajax({
			...
			success:function(data){
				setTimeout(function() {
    				...
    				jQuery("#saved-data :text").prop('disabled', true);
  	...
  	jQuery('.member-get-address').click(function(){
  		getAddress();
  	});
    
  	jQuery('#member-save-location').click(function(){
		getLatLong();	
    });
	...
	jQuery('#wppl-address-tab').click(function(){
  		showAddressTab();
  	});
  	
	jQuery('#wppl-latlong-tab').click(function(){
  		showLatLongTab();
  	});
	
	function showAddressTab() {	
		if ( 'active' != jQuery(this).attr('class') ) {
			jQuery('div.icons-tab').hide();
