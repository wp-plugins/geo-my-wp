...
jQuery(window).load(function(){ 
	...
	for (i = 0; i < fMapArgs.locations.length; i++) {  
		...
		google.maps.event.addListener(mMarkers[i], 'click', openMemberWindow(mMarkers, i));
	...
	// show info window  
	function openMemberWindow(mMarkers,i) {
		return function() {
			if (fliw) {
				fliw.close();
				fliw = null;
			...
			locMarker = new google.maps.Marker({
				position: mMarkers[i].position,
				map: membersMap,
				icon: mapIcon,
			});
			fliw = new google.maps.InfoWindow({
				content: getFLIWContent(i),
			});
			fliw.open(membersMap, locMarker); 			
		}
	}
