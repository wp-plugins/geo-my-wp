...
jQuery(window).load(function(){ 
	...
	for (i = 0; i < mainMapArgs.locations.length; i++) {  
		...
		google.maps.event.addListener(ptMarkers[i], 'click', openPtWindow(ptMarkers, i));
		setTimeout(dropMarker(ptMarkers,i), i * 250); 	
	}
	...
	marker = new google.maps.Marker({
		...
		// show info window  
		function openPtWindow(ptMarkers,i) {
			return function() {
				if (ptiw) {
					ptiw.close();
					ptiw = null;
				}
				ptMarker = new google.maps.Marker({
					position: ptMarkers[i].position,
					map: ptMap,
					icon: mapIcon,
				});
				ptiw = new google.maps.InfoWindow({
					content: getPTIWContent(i),
				});
	...
	// drop marker 								
	function dropMarker(ptMarkers, i) {
		return function() {
