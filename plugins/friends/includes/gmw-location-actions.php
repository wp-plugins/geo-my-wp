...
function gmw_location_register_scripts(){
	...
	function gmw_fl_register_main_map($main_map) {
		...
		$main_map =  wp_register_script( 'gmw-fl-map', GMW_FL_URL . '/includes/js/main-map.js',array(),false,true );
		return $main_map;
	
	}
	add_filter('gmw_fl_register_main_map', 'gmw_fl_register_main_map', 99, 1 );
	
	echo apply_filters('gmw_fl_register_main_map', $main_map );
