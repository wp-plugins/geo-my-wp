...
function gmw_all_users_transient() {
	global $wpdb;
	$gmwMembers = $wpdb->get_results("SELECT gmwusers.member_id, gmwusers.lat, gmwusers.long, gmwusers.street, gmwusers.apt, gmwusers.city, gmwusers.state, gmwusers.zipcode, gmwusers.country, gmwusers.address,
			wpusers.ID, wpusers.display_name
			FROM wppl_friends_locator gmwusers INNER JOIN ".$wpdb->prefix."users wpusers ON gmwusers.member_id = wpusers.ID", ARRAY_A);
		
	set_transient( 'gmwAllUsers', $gmwMembers, 60*60 );
}
...
function gmw_member_location($member) {
	...
	if ( isset( $member_info ) && !empty( $member_info ) && $member_info != 0 ) {
		 ...
		 */
		$show_address = $member_info[0]['street'] .' '. $member_info[0]['city'] .' ' . $member_info[0]['state'] .' ' . $member_info[0]['zipcode'] .' ' . $member_info[0]['country'];
    	...
    	$member_map = false;
    	$member_map .='';	
    	$member_map .=	'<div id="gmw-single-member-sc-wrapper-'.$member_info[0]['member_id'].'" class="gmw-single-member-sc-wrapper">';
    	...
    	return apply_filters('gmw_fl_single_member_location', $member_map);
