...
jQuery(document).ready(function(){ 
	...
	var ptMap = new google.maps.Map(document.getElementById('gmw-pt-map-' + ptMapArgs.form_id), {
		zoom: parseInt(ptMapArgs.zoom_level),
		panControl: true,
  		zoomControl: true,
  		mapTypeControl: true,
	...
	if ( ptMapArgs.your_lat != false && ptMapArgs.your_long != false ) {
		...
		if ( ptMapArgs.your_location_icon ) yLMapIcon = ptMapArgs.main_icons_folder + '/your-location-icons/' + ptMapArgs.your_location_icon; else yLMapIcon = 'http://maps.google.com/mapfiles/ms/icons/blue-dot.png';
	...
	function getPTIWContent(i) {
		...
		content +=				'<tr><td><span>Address: </span>' + ptLocations[i]['address'] + '</td></tr>';
		if ( ptMapArgs.units_array != false ) {
			content +=				'<tr><td><span>Distance: </span>' + ptLocations[i]['distance'] + ' ' + ptMapArgs.units_array['name'] + '</td></tr>';
		}
		content +=				'<tr><td><span>Phone: </span>' + ptLocations[i]['phone'] + '</td></tr>';
		content +=				'<tr><td><span>Fax: </span>' + ptLocations[i]['fax'] + '</td></tr>';
		content +=				'<tr><td><span>Email Address: </span>' + ptLocations[i]['email'] + '</td></tr>';
		content +=				'<tr><td><span>Website: </span><a href="http://' + ptLocations[i]['website'] + '" target="_blank">' + ptLocations[i]['website'] + '</a>' + '</td></tr>';
		content +=			'</table>';
		content +=		'</div>';
		content +=  '</div>';
		return content;
