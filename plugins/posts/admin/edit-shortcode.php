		...
		<div class="gmw-ssh wppl-settings">
			...
			<span class="help-btn-tooltip-wrapper">
				...
				<span class="wppl-help-message">
					<?php _e('Type the title for the address field of the search form. for example "Enter your address". this title wll be displayed either next to the address input field or within if you check the checkbox for it. ', 'GMW'); ?>
							...
							if (is_taxonomy_hierarchical($tax)) :
								echo '<div style="border-bottom:1px solid #eee;padding-bottom: 10px;margin-bottom: 10px;" class="gmw-single-taxonomie">';													
									...
									echo '<span id="gmw-st-wrapper">';
										echo '<input type="radio" class="gmw-st-btns radio-na" name="wppl_shortcode[' .$e_id .'][taxonomies]['.$tax.'][style]" value="na" '; if( isset($option['taxonomies'][$tax]['style']) && $option['taxonomies'][$tax]['style'] == 'na' ) echo  "checked=checked"; echo  ' style="margin-left: 10px; " />Exclude';
										echo '<input type="radio" class="gmw-st-btns" name="wppl_shortcode[' .$e_id .'][taxonomies]['.$tax.'][style]" value="drop" '; if( isset($option['taxonomies'][$tax]['style']) && $option['taxonomies'][$tax]['style'] == 'drop' ) echo  "checked=checked"; echo  ' style="margin-left: 10px; " />Dropdown';
									echo '</span>';
								...
								<div class="gmw-ssb">
									<select name="<?php echo 'wppl_shortcode[' .$e_id .'][results_page]'; ?>">
										...
										<?php foreach ($pages_s as $page_s) {
											echo '<option value="' . get_bloginfo('wpurl') . "/" . $page_s->post_name.'"'; if ( isset($option['results_page']) && $option['results_page'] == get_bloginfo('wpurl') . "/" . $page_s->post_name ) echo 'selected="selected"'; echo '>'. $page_s->post_title . '</option>';
										} ?>
									</select>			
								</div>
							</li>
