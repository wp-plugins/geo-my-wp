	...
	if ( $gmw_query->have_posts() ):
		...
		*/
		...
		if ( $gmw['results_type'] != 'posts' || isset( $gmw['single_map']) ) :
			...
			$gmw['post_types_icons']  = ( isset( $gmw_options['post_types_icons'] ) ) ? $gmw_options['post_types_icons'] : false;
		
			$gmw = apply_filters('gmw_pt_settings_before_map', $gmw, $gmw_query);
			$gmw_query = apply_filters('gmw_pt_posts_loop_before_map',$gmw_query, $gmw);
