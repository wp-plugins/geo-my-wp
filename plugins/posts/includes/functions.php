<?php 
/**
 * GMW PT function - Query all posts with location and save in transien
 * @version 1.0
 * @author Eyal Fitoussi
 */
function gmw_all_posts_transient() {
	global $wpdb;
	$gmwLocations = $wpdb->get_results("SELECT gmwlocations.post_id, gmwlocations.street, gmwlocations.apt, gmwlocations.city, gmwlocations.state, gmwlocations.zipcode, gmwlocations.country, gmwlocations.address,
			wplocations.ID, wplocations.post_title, wplocations.post_type
			FROM " . $wpdb->prefix . "places_locator gmwlocations INNER JOIN " . $wpdb->prefix . "posts wplocations ON gmwlocations.post_id = wplocations.ID", ARRAY_A);
		
	set_transient( 'gmwAllLocations', $gmwLocations, 60*60 );
}
	...
	if ( $post_id != 0 || is_single() ) {
		...
		*/
		...
		if ( !isset($post_info) || $post_info !=0 ) {
			...
			$single_map .= 					'</form>';
			$single_map .= 				'</div>';
			$single_map .= 				'<a href="#" id="gmw-single-post-sc-directions-trigger-'.$mmc.'"  class="gmw-single-post-sc-directions-trigger">Get Directions</a>';
			$single_map .=			'</div>';
			...
			$single_map .=		'<div class="gmw-address"><span>' . __('Address: ','GMW'); $single_map .= '</span>'; $single_map .= ( isset($post_info[0]['address']) && !empty($post_info[0]['address'])) ? $post_info[0]['address'] : __('N/A','GMW'); $single_map .= '</div>';
			$single_map .=		'<div class="gmw-phone"><span>'. __('Phone: ','GMW'); $single_map .= '</span>'; $single_map .= ( isset($post_info[0]['phone']) && !empty($post_info[0]['phone'])) ? $post_info[0]['phone'] : __('N/A','GMW'); $single_map .= '</div>';
			$single_map .=		'<div class="gmw-fax"><span>'. __('Fax','GMW'); $single_map .= '</span>'; $single_map .= ( isset($post_info[0]['fax']) && !empty($post_info[0]['fax'])) ? $post_info[0]['fax'] : __('N/A','GMW'); $single_map .= '</div>';
			$single_map .=		'<div class="gmw-email"><span>'. __('Email: ','GMW'); $single_map .= '</span>'; $single_map .= ( isset($post_info[0]['email']) && !empty($post_info[0]['email'])) ? $post_info[0]['email'] : __('N/A','GMW'); $single_map .= '</div>';
			$single_map .=		'<div class="gmw-website"><span>'. __('Website: ','GMW'); $single_map .= '</span>'; $single_map .= ( isset($post_info[0]['website']) && !empty($post_info[0]['website']) ) ? '<a href="http://' . $post_info[0]['website']. '" target="_blank">' .$post_info[0]['website']. '</a>' : "N/A"; $single_map .= '</div>';
			$single_map .= '</div>';
			endif;
