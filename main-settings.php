<table class="widefat fixed" style="margin-bottom:10px;">
	...
	<table class="widefat fixed">
		...
		<tr>
			...
			<td>
				<p>
				<input id="wppl-locate-message" name="wppl_fields[auto_locate] " type="checkbox" value="1" <?php if ($options['auto_locate'] == 1) {echo "checked";}; ?>/>
				<?php echo _e('Yes','wppl'); ?>
				&nbsp;&nbsp;&#124;&nbsp;&nbsp;
				<input id="wppl-locate-message" name="wppl_fields[locate_message] " type="checkbox" value="1" <?php if ($options['locate_message'] == 1) {echo "checked";}; ?>/>
		...
		<tr>
			...
			<td>
				<p>
				<input id="wppl-auto-search" name="wppl_fields[auto_search] " type="checkbox" value="1" <?php if ($options['auto_search'] == 1) {echo "checked";}; ?>/>
				<?php echo _e('Yes','wppl'); ?>
				&nbsp;&nbsp;&#124;&nbsp;&nbsp;
				<?php echo _e('Radius','wppl'); ?>		
				<input type="text" id="wppl-auto-radius" name="wppl_fields[auto_radius]" SIZE="1" onkeyup="this.value=this.value.replace(/[^\d]/,'')" value="<?php echo ($options['auto_radius']) ? $options['auto_radius'] : "50"; ?>" />	
				&nbsp;&nbsp;&#124;&nbsp;&nbsp;
				<select id="wppl-auto-units" name="wppl_fields[auto_units]">
					<option value="imperial" <?php echo ($options['auto_units'] == "imperial") ? "selected" : ""; ?>>Miles</option>
					<option value="metric" <?php echo ($options['auto_units'] == "metric") ? "selected" : ""; ?>>Kilometers</option>
				</select>
				</p>
			</td>
		</tr>
		<tr>
			...
			<td>
				<p>
				<span style="float:left"><input name="wppl_fields[show_locator_icon] " type="checkbox" value="1" <?php if ($options['show_locator_icon'] == 1) {echo "checked";}; ?>/>
					<?php echo _e('Yes','wppl'); ?>
					&nbsp;&nbsp;&#124;&nbsp;&nbsp;
				</span>
				<span style="width:365px;float:left;margin-left:10px;">	
					...
					<span style="float:left;">
						<input type="radio" name="wppl_fields[locator_icon]" value="<?php echo basename($locator_icon); ?>" <?php echo ($options['locator_icon'] == basename($locator_icon) ) ? "checked" : ""; ?> />
		...
		<tr>
			...
			<td>
				<p>
				<input name="wppl_fields[wider_search] " type="checkbox" value="1" <?php if ($options['wider_search'] == 1) {echo "checked";}; ?>/>
				<?php echo _e('Yes','wppl'); ?>
				&nbsp;&nbsp;&#124;&nbsp;&nbsp;
				<?php echo _e('Distance value: ','wppl'); ?>
				<input id="wider-search" name="wppl_fields[wider_search_value]" size="5" type="text" value="<?php echo $options[wider_search_value]; ?>" /></p>
			</td>
		</tr>
		...
		<tr>
			...
			<td>
				<p>
				<input name="wppl_fields[front_autocomplete] " type="checkbox" value="1" <?php if ($options['front_autocomplete'] == 1) {echo "checked";} echo "disabled" ?>/>
		...
		<tr>
			...
			<div class="wppl-settings">
				<p>
				<span>
					<span><?php _e('Admin javascript conflict:', 'wppl'); ?></span>
					<span><a href="#" class="wppl-help-btn"><img src="<?php echo plugins_url('/geo-my-wp/images/help-btn.png'); ?>" width="25px" height="25px" style="float:left;" /></a></span>
				</span>
				<div class="clear"></div>
					<span class="wppl-help-message">
						<?php _e('Check this box if you are having a javascript conflict in the "new/update post" page, it might fix it. one known conflict causes CouponPress tabs disappear.', 'wppl'); ?>
					</span>
				</p>
			</div>	
			</td>
			<td>
				<p>
				<input name="wppl_fields[java_conflict] " type="checkbox" value="1" <?php if ($options['java_conflict'] == 1) {echo "checked";}; ?>/>
				<?php echo _e('Yes','wppl'); ?></p>
			</td>
		</tr>
		<tr>
			<td>
		...
		<tr>
			...
			<td>
				<p>
				<input name="wppl_fields[mandatory_address] " type="checkbox" value="1" <?php if ($options['mandatory_address'] == 1) {echo "checked";}; ?>/>
				<?php echo _e('Yes','wppl'); ?>
				</p>
			</td>
		</tr>
		...
		<tr>
			...
			<td>
				<p>
				<input type="checkbox" name="wppl_fields[use_theme_color]" value="1" <?php echo ($options['use_theme_color']) ? 'checked="checked"' : ''; ?>" />
				<?php echo _e('Yes','wppl'); ?>
				&nbsp;&nbsp;&#124;&nbsp;&nbsp;
				<input type="text" id="wppl-theme-color" name="wppl_fields[theme_color]" value="<?php echo ($options['theme_color']) ? $options['theme_color'] : "#2E738F"; ?>" />
		...
		<tr>
			...
			<td>
				<p><input  type="text" id="api_key" name="wppl_fields[no_results]" size="40" value="<?php echo ($options[no_results]) ? $options[no_results] : "No results were found."; ?>" /></p>
			</td>
		</tr>
		<tr>
			<td>
		...
		<tr>
			...
			<select name="wppl_fields[friends_results_page]" style="float:left" <?php echo(!$wppl_on['friends']) ? "disabled" : "";?>>
				<?php foreach ($pages_s as $page_s) {
				echo '<option value="'.$page_s->post_name.'"'; echo ($options[friends_results_page] == $page_s->post_name ) ? 'selected="selected"' : "" ; echo '>'. $page_s->post_title . '</option>';
				}
			?>
			</select>
		...
		<tr>
			...
			<td  class="wppl-premium-version-only">
				<p>
				<input name="wppl_fields[per_member_icon] " type="checkbox" value="1" <?php if ($options['per_member_icon'] == 1) {echo "checked";}; echo( !$wppl_on['friends'] ) ? "disabled" : "";?>/>
				<?php echo _e('Yes','wppl'); ?>
				</p>
			</td>
		</tr>
