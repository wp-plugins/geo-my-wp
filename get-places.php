...
function wppl_shortcode($params) {
	global $post_types, $options, $wppl;
	$post_types = array();
	...
	$results_page = ($friends_search) ? $options['friends_results_page'] : $options['results_page'];
			...
			<input type="hidden" name="pagen" value="0" />
				...
				<div class="wppl-address-wrapper">        		
					...
					<div class="wppl-address-field-wrapper">
						...
						<button type="button" value="<?php echo $params[form]; ?>" onClick="getLocation();formId=this.value;" class="wppl-locate-me-btn"><img src="<?php echo plugins_url('images/locator-images/'.$options['locator_icon'], __FILE__); ?>" /></button>
					</div>
					<?php if($options['show_locator_icon']) { ?>
...
<?php 	
	if (empty($params[form_only])) {
		if ($friends_search) 
			wppl_bp_query_results ($params, $wppl, $options);
		else 
			wppl_get_results($params, $wppl, $options);	
    }
...
function wppl_get_results($params, $wppl, $options) {

	if(!empty( $_GET['action'] ) &&  $_GET['action'] == "wppl_post") {		
 		
 		global $wppl, $wpdb, $post, $org_address, $options, $lat , $long, $distance_between, $post_types, $unit_a, $options,$per_page, $posts_within, $total_rows, $from_page, $wppl_units; 
		...
		global $wpdb, $post, $distance_between, $post_types, $lat , $long, $org_address, $unit_a, $options,$per_page, $posts_within, $total_rows, $from_page, $wppl_units; 
