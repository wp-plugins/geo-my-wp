...
function wppl_shortcode_single($params_s) {
...
function get_results_within( $org_address, $radius ) {
	...
	global $posts_within, $map_data, $plugin_map, $total_rows, $by_driving, $mikm_n, $posts_within, $map_type, $result_style;
	...
	$pages = $total_rows[0]/$per_page; //get number of pages //
		if ($pages > 1) {
			...
			if ($page_n > 0) {		
				...
				echo '</div>';
				if ($page_n < ($pages -1)) {
					echo '<div class="next-btn" id="fg"><a href="' .add_query_arg('page', ($page_n + 1)) .'" onclick="document.wppl_form.submit();">&#187;</a></div>';
				 }			
			echo '</div>';
		}
	}	
	...
	global $show_map, $which_map, $map_height, $map_width, $thumb_size, $directions;
