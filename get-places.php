...
function wppl_shortcode($params) {	
	...
	//// DISPLAY SEARCH FORM ////	
	echo	'<div class="wppl-'; if($params[form_only] == 1) { echo "widget-";}; echo 'form-wrapper">';
	echo 	'<form class="wppl-'; if($params[form_only] == 1) { echo "widget-";}; echo 'form" name="wppl_form" id="wppl-form" action="'; if($params[form_only] == 1) { bloginfo('wpurl'); echo "/" . $options['results_page'];} echo '" method="get">';
...
function get_results_within() {	
	...
	$sql ="
		SELECT SQL_CALC_FOUND_ROWS wposts.post_id, wposts.lat, wposts.long, wposts.address, wposts.post_title, wposts.phone, wposts.fax, wposts.email, wposts.website, ROUND(" . $mikm . "* acos( cos( radians( $lat ) ) * cos( radians( wposts.lat ) ) * cos( radians( wposts.long ) - radians( $long ) ) + sin( radians( $lat ) ) * sin( radians( wposts.lat) ) ),1 )  AS distance 
	...
	$pages = $total_rows[0]/$per_page; //get number of pages //
		if ($pages > 1) {
			...
			if ($page_n > 0) {		
					echo '<div class="prev-btn"><a href="' .add_query_arg('page', ($page_n - 1)) .'" onclick="document.wppl_form.submit();">&#171;</a></div>';
				} 
				...
				echo '</div>';
				if ($page_n < ($pages -1)) {
					echo '<div class="next-btn"><a href="' .add_query_arg('page', ($page_n + 1)) .'" onclick="document.wppl_form.submit();">&#187;</a></div>';
				 }			
			echo '</div>';
		}
	}	
	...
	echo		'<h2 class="wppl-h2"> Showing ' . count($posts_within) . ' out of ' . $total_rows[0] . ' results within ' . $radius . ' ' . $mikm_n . ' from ' . $_GET['wppl_address'] . '</h2>';
				pagination_links();
