...
class GEO_my_WP {
    ...
    public function frontend_register_scripts() {
        ...
        if ( !wp_script_is( 'google-maps', 'registered' ) ) {
            wp_register_script( 'google-maps', ( is_ssl() ? 'https' : 'http' ) . '://maps.googleapis.com/maps/api/js?libraries=places'.$googleApi.'&sensor=false&ver=3.13'.$region.$language, array( 'jquery' ), false );
    	}
    ...
    public function gmw( $params ) {
        ...
        $this->form 					= array();
        $this->form['submitted']		= ( isset( $_GET['action'] ) && $_GET['action'] == "gmw_post" ) ? true : false;
        $this->form['results_page_use'] = ( isset( $params['form'] ) && $params['form'] == 'results' ) ? true : false;
        $this->form['in_widget'] 		= ( isset( $params['widget'] ) ) ? true : false;
        $this->form['ul_lat']			= ( isset( $_COOKIE['gmw_lat'] ) ) ? urldecode( $_COOKIE['gmw_lat'] ) : false;
        $this->form['ul_lng']			= ( isset( $_COOKIE['gmw_lng'] ) ) ? urldecode( $_COOKIE['gmw_lng'] ) : false;
        $this->form['ul_address']		= ( isset( $_COOKIE['gmw_address'] ) ) ? urldecode( $_COOKIE['gmw_address'] ) : false;
        ...
        //display map anywere on the page using the map shortcode
        if ( isset( $params['map'] ) ) {

            $this->form = array_merge( $this->form, $this->forms[$params['map']] );
        		
           	//if form is not set to be used with the shortcode return
        ...
        //display results using the results shortcode			
        } elseif ( $params['form'] == 'results' ) {
        	
        	//make sure form was submitted
        	if ( isset( $_GET['action'] ) && $_GET['action'] == "gmw_post" ) {

                $this->form = array_merge( $this->form, $this->forms[$_GET['gmw_form']] );
            
                do_action( 'gmw_' . $this->form['form_type'] . '_shortcode', $this->form );
            }

        //display results when form submitted
        } else {

        	$this->form = array_merge( $this->form, $this->forms[$params['form']] );
            
            if ( isset( $this->form['search_results']['results_page'] ) && !empty( $this->form['search_results']['results_page'] ) ) {
